            ...
            >
              ...
              <div className="mt-auto">
                ...
                <div className="flex gap-4">
                  <div className="w-8 h-8 rounded-full bg-surface border border-border flex items-center justify-center text-muted hover:text-gold transition-colors">
                    <Anchor size={14} />
                  </div>
                  <div className="w-8 h-8 rounded-full bg-surface border border-border flex items-center justify-center text-muted hover:text-gold transition-colors">
                    <Truck size={14} />
...
export const RouteCard = ({ route, onClick }: { route: Route; onClick: () => void }) => {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.97 }}
      onClick={onClick}
      className={`bg-deep border-l-[3px] ${modeColor} p-5 cursor-pointer transition-shadow hover:shadow-[0_8px_32px_rgba(0,0,0,0.4)]`}
    >
      <div className="flex justify-between items-start mb-4">
        <ModeIcon size={20} className={iconColor} />
        <span className={`ui-label px-2 py-1 border ${route.bookingType === 'INSTANT' ? 'bg-success text-ink border-success' : 'border-gold text-gold'}`}>
          {route.bookingType}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
        <div>
          ...
          <p className="text-muted text-sm leading-relaxed font-body italic">
            "The visual weight of a dive magazine, the tactility of weathered nautical charts."
          </p>
        </div>
