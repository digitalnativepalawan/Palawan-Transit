    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center px-6">
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-deep border border-border p-10 max-w-2xl w-full relative z-10">
        ...
        <div className="grid grid-cols-2 gap-6 mb-10">
          ...
          <div className="space-y-1"><label className="ui-label text-muted">TRANSPORT MODE</label><select value={formData.mode} onChange={e => setFormData({...formData, mode: e.target.value as any})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none"><option value="SHUTTLE">SHUTTLE</option><option value="BANGKA">BANGKA</option><option value="PRIVATE">PRIVATE VEHICLE</option><option value="ISLAND_HOPPING">ISLAND HOPPING</option></select></div>
        ...
        <div className="flex gap-4"><button onClick={onClose} className="flex-1 py-4 border border-border text-muted ui-label text-[10px] hover:text-white transition-colors">CANCEL</button><button onClick={() => onSave(formData)} className="flex-1 py-4 bg-gold text-ink ui-label text-[10px] font-bold tracking-[0.2em]">SAVE ROUTE</button></div>
      </motion.div>
    </motion.div>
  );
};
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center px-6">
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-deep border border-border p-10 max-w-md w-full relative z-10">
        ...
        <div className="flex gap-4"><button onClick={onClose} className="flex-1 py-4 border border-border text-muted ui-label text-[10px] hover:text-white transition-colors">CANCEL</button><button onClick={() => onSave(formData)} className="flex-1 py-4 bg-gold text-ink ui-label text-[10px] font-bold tracking-[0.2em]">SAVE OPERATOR</button></div>
      </motion.div>
    </motion.div>
  );
};
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center px-6">
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-deep border border-border p-10 max-w-2xl w-full relative z-10">
        ...
        <div className="flex gap-4"><button onClick={onClose} className="flex-1 py-4 border border-border text-muted ui-label text-[10px] hover:text-white transition-colors">CANCEL</button><button onClick={() => onSave(formData)} className="flex-1 py-4 bg-gold text-ink ui-label text-[10px] font-bold tracking-[0.2em]">CREATE BOOKING</button></div>
      </motion.div>
    </motion.div>
  );
};
...
const DashboardView = ({ bookings, routes, onUpdateStatus, isOperatorPortal }: any) => {
  ...
  const handleVerify = () => {
    ...
    if (booking) {
      ...
      else { onUpdateStatus(booking.id, 'ACCEPTED'); setVerifyCode(''); }
    } else setVerifyError('INVALID REFERENCE CODE');
  };
  return (
    <div className="space-y-6 lg:space-y-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
        {stats.map((stat, i) => <div key={i} className="bg-[#081221] border border-white/10 p-6"><p className="ui-label text-[9px] text-muted mb-4 tracking-[0.2em]">{stat.label}</p><p className={`text-3xl lg:text-4xl font-ui mb-2 ${stat.color}`}>{stat.value}</p><p className="ui-label text-[8px] text-muted opacity-60">{stat.sub.toUpperCase()}</p></div>)}
      </div>
      {isOperatorPortal && <div className="bg-gold/5 border border-gold/20 p-8"><div className="max-w-xl"><h3 className="text-2xl text-white italic mb-2">Verify Reference Code</h3><p className="ui-label text-muted text-[10px] mb-6 tracking-[0.2em]">ENTER THE CUSTOMER'S PIN TO CLAIM THE BOOKING</p><div className="flex gap-4"><div className="flex-1 relative"><input value={verifyCode} onChange={e => setVerifyCode(e.target.value)} placeholder="PT-2026-XXXXX" className="w-full bg-surface border border-white/10 p-4 text-white font-ui tracking-widest focus:border-gold outline-none" />{verifyError && <p className="absolute -bottom-6 left-0 text-danger ui-label text-[8px] tracking-widest">{verifyError}</p>}</div><button onClick={handleVerify} className="px-8 bg-gold text-ink ui-label text-[10px] font-bold tracking-[0.2em] hover:bg-white transition-all">ACCEPT PIN</button></div></div></div>}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="lg:col-span-2 bg-[#081221] border border-white/10 p-6 lg:p-8"><h3 className="ui-label text-[10px] text-white tracking-[0.2em] mb-8">REVENUE - LAST 7 DAYS</h3><div className="h-[250px] lg:h-[300px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={[{ name: 'MON', rev: 28400 }, { name: 'TUE', rev: 32100 }, { name: 'WED', rev: 38450 }, { name: 'THU', rev: 30200 }, { name: 'FRI', rev: 42000 }, { name: 'SAT', rev: 45600 }, { name: 'SUN', rev: 41200 }]}><CartesianGrid strokeDasharray="3 3" stroke="#ffffff10" vertical={false} /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#8E9299', fontSize: 10 }} dy={10} /><YAxis hide /><Tooltip cursor={{ fill: '#ffffff05' }} contentStyle={{ backgroundColor: '#081221', border: '1px solid #ffffff10' }} itemStyle={{ color: '#C9943A' }} /><Bar dataKey="rev" fill="#C9943A" radius={[2, 2, 0, 0]} barSize={40} /></BarChart></ResponsiveContainer></div><div className="flex justify-between mt-8 pt-8 border-t border-white/5"><div><p className="ui-label text-[8px] text-muted mb-1">7-DAY TOTAL</p><p className="text-xl lg:text-2xl text-gold font-ui">₱214,800</p></div><div className="text-right"><p className="ui-label text-[8px] text-muted mb-1">AVG / DAY</p><p className="text-xl lg:text-2xl text-white font-ui">₱30,685</p></div></div></div>
        <div className="bg-[#081221] border border-white/10 p-6 lg:p-8"><div className="flex justify-between items-center mb-8"><h3 className="ui-label text-[10px] text-white tracking-[0.2em]">PENDING REQUESTS</h3><button className="ui-label text-[8px] text-gold hover:underline">VIEW ALL →</button></div><div className="space-y-4">{pendingBookings.slice(0, 4).map((req: any) => { const route = routes.find((r: any) => r.id === req.routeId); return (<div key={req.id} className="flex items-center justify-between p-4 border border-white/5 hover:border-white/20 transition-all group"><div><p className="text-white text-sm italic mb-1">{req.customerName}</p><p className="ui-label text-[8px] text-muted">{route ? `${route.from} → ${route.to}` : 'Unknown Route'} · {req.date} · {req.seats} seats</p></div><div className="flex gap-2 opacity-40 group-hover:opacity-100 transition-opacity"><button onClick={() => onUpdateStatus(req.id, 'ACCEPTED')} className="w-8 h-8 lg:w-10 lg:h-10 border border-white/10 flex items-center justify-center text-white hover:bg-white hover:text-ink transition-all"><Check size={14} /></button><button onClick={() => onUpdateStatus(req.id, 'CANCELLED')} className="w-8 h-8 lg:w-10 lg:h-10 border border-white/10 flex items-center justify-center text-white hover:bg-danger hover:border-danger transition-all"><X size={14} /></button></div></div>);})}{pendingBookings.length === 0 && <div className="text-center py-12 border border-dashed border-white/5"><p className="ui-label text-[10px] text-muted italic">NO PENDING REQUESTS</p></div>}</div></div>
      </div>
    </div>
  );
};
...
const BookingsView = ({ bookings, onManualBooking, onUpdateStatus, isOperatorPortal }: any) => {
  ...
  const filtered = bookings.filter((b: any) => (filter === 'ALL' || b.status === filter) && (b.referenceCode.toLowerCase().includes(search.toLowerCase()) || b.customerName.toLowerCase().includes(search.toLowerCase())));
  return (
    <div className="space-y-6 lg:space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"><div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto"><div className="relative w-full sm:w-80"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-muted" size={16} /><input placeholder="Search bookings, names, refs..." value={search} onChange={(e) => setSearch(e.target.value)} className="bg-[#081221] border border-white/10 pl-12 pr-6 py-3 ui-label text-[11px] text-white w-full focus:border-gold outline-none" /></div><select value={filter} onChange={(e) => setFilter(e.target.value)} className="bg-[#081221] border border-white/10 text-white text-[11px] py-3 px-6 outline-none focus:border-gold ui-label"><option value="ALL">STATUS: ALL</option><option value="PENDING">STATUS: PENDING</option><option value="CONFIRMED">STATUS: CONFIRMED</option><option value="ACCEPTED">STATUS: ACCEPTED</option><option value="CANCELLED">STATUS: CANCELLED</option><option value="COMPLETED">STATUS: COMPLETED</option></select></div>{!isOperatorPortal && <button onClick={onManualBooking} className="w-full sm:w-auto bg-gold text-ink px-8 py-3 ui-label text-[11px] font-bold tracking-[0.2em] flex items-center justify-center gap-2"><Plus size={16} /> MANUAL BOOKING</button>}</div>
      <div className="lg:hidden space-y-4">{filtered.length === 0 ? <div className="bg-[#081221] border border-white/10 p-12 text-center text-muted ui-label italic">No bookings found</div> : filtered.map((b: any) => (<div key={b.id} className="bg-[#081221] border border-white/10 p-6 space-y-4"><div className="flex justify-between items-start"><div><p className="font-ui text-gold text-sm mb-1">{b.referenceCode}</p><p className="text-white text-lg italic">{b.customerName}</p></div><span className={`ui-label text-[8px] px-2 py-1 ${b.status === 'CONFIRMED' || b.status === 'ACCEPTED' ? 'bg-success/20 text-success' : b.status === 'PENDING' ? 'bg-gold/20 text-gold' : 'bg-danger/20 text-danger'}`}>{b.status}</span></div><div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5"><div><p className="ui-label text-[8px] text-muted mb-1">ROUTE</p><p className="text-white text-xs italic">{b.routeId}</p></div><div><p className="ui-label text-[8px] text-muted mb-1">DATE</p><p className="text-white text-xs">{b.date}</p></div><div><p className="ui-label text-[8px] text-muted mb-1">TOTAL</p><p className="text-white font-ui">₱{b.totalPrice}</p></div><div className="flex justify-end items-end gap-2">{b.status === 'PENDING' && <><button onClick={() => onUpdateStatus(b.id, 'ACCEPTED')} className="p-2 text-success hover:bg-success/10 transition-colors"><Check size={16} /></button><button onClick={() => onUpdateStatus(b.id, 'CANCELLED')} className="p-2 text-destructive hover:bg-destructive/10 transition-colors"><X size={16} /></button></>}<button className="p-2 text-muted hover:text-white"><MoreVertical size={16} /></button></div></div></div>))}</div>
      <div className="hidden lg:block bg-[#081221] border border-white/10 overflow-hidden"><table className="w-full text-left"><thead><tr className="border-b border-white/10 bg-white/5"><th className="px-6 py-4 ui-label text-[9px] text-muted tracking-[0.2em]">REF</th><th className="px-6 py-4 ui-label text-[9px] text-muted tracking-[0.2em]">PASSENGER</th><th className="px-6 py-4 ui-label text-[9px] text-muted tracking-[0.2em]">ROUTE</th><th className="px-6 py-4 ui-label text-[9px] text-muted tracking-[0.2em]">DATE</th><th className="px-6 py-4 ui-label text-[9px] text-muted tracking-[0.2em]">STATUS</th><th className="px-6 py-4 ui-label text-[9px] text-muted tracking-[0.2em]">TOTAL</th><th className="px-6 py-4 ui-label text-[9px] text-muted tracking-[0.2em] text-right">ACTIONS</th></tr></thead><tbody className="divide-y divide-white/5">{filtered.map((b: any) => (<tr key={b.id} className="hover:bg-white/5 transition-colors"><td className="px-6 py-4 font-ui text-gold text-xs">{b.referenceCode}</td><td className="px-6 py-4"><p className="text-white text-sm italic">{b.customerName}</p><p className="ui-label text-[8px] text-muted">{b.customerPhone}</p></td><td className="px-6 py-4 text-white/60 text-xs italic">{b.routeId}</td><td className="px-6 py-4 text-white/60 text-xs">{b.date}</td><td className="px-6 py-4"><span className={`ui-label text-[8px] px-2 py-1 ${b.status === 'CONFIRMED' || b.status === 'ACCEPTED' ? 'bg-success/20 text-success' : b.status === 'PENDING' ? 'bg-gold/20 text-gold' : 'bg-danger/20 text-danger'}`}>{b.status}</span></td><td className="px-6 py-4 text-white font-ui">₱{b.totalPrice.toLocaleString()}</td><td className="px-6 py-4 text-right"><div className="flex justify-end gap-2">{b.status === 'PENDING' && <><button onClick={() => onUpdateStatus(b.id, 'ACCEPTED')} className="p-2 text-success hover:bg-success/10 transition-colors"><Check size={16} /></button><button onClick={() => onUpdateStatus(b.id, 'CANCELLED')} className="p-2 text-destructive hover:bg-destructive/10 transition-colors"><X size={16} /></button></>}<button className="p-2 text-muted hover:text-white"><MoreVertical size={16} /></button></div></td></tr>))}</tbody></table></div>
    </div>
  );
};

const RoutesView = ({ routes, onAdd, onEdit, onDelete }: any) => (
  <div className="space-y-6 lg:space-y-8"><div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"><h3 className="ui-label text-[10px] text-gold tracking-[0.2em]">{routes.length} ACTIVE ROUTES</h3><button onClick={onAdd} className="w-full sm:w-auto bg-gold text-ink px-8 py-3 ui-label text-[11px] font-bold tracking-[0.2em] flex items-center justify-center gap-2"><Plus size={16} /> CREATE NEW ROUTE</button></div><div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">{routes.map((r: any) => (<div key={r.id} className="bg-[#081221] border border-white/10 p-6 lg:p-8 flex justify-between items-start group hover:border-gold/50 transition-all"><div className="flex-1"><div className="flex items-center gap-3 mb-4">{r.mode.startsWith('SHUTTLE') ? <Truck size={16} className="text-gold" /> : r.mode === 'PRIVATE_4X4' ? <Truck size={16} className="text-danger" /> : (r.mode === 'BANGKA' || r.mode === 'ISLAND_HOPPING') ? <Anchor size={16} className="text-seafoam" /> : <Waves size={16} className="text-gold" />}<span className="ui-label text-[9px] text-muted tracking-[0.2em]">{r.mode}</span><span className="w-1 h-1 rounded-full bg-success" /><span className="ui-label text-[9px] text-success tracking-[0.2em]">LIVE</span></div><h4 className="text-2xl lg:text-3xl text-white italic mb-2">{r.from} → {r.to}</h4><p className="ui-label text-[10px] text-muted tracking-widest mb-6">{r.operator} · {r.duration} · {r.departureTime}</p><div className="flex gap-4"><button onClick={() => onEdit(r)} className="ui-label text-[9px] text-gold hover:underline">EDIT DETAILS</button><button onClick={() => onDelete(r.id)} className="ui-label text-[9px] text-danger hover:underline">DELETE</button></div></div><div className="text-right ml-4"><p className="text-2xl lg:text-3xl text-white font-ui mb-1">₱{r.price}</p><p className="ui-label text-[8px] text-muted">{r.bookingType} BOOKING</p></div></div>))}</div></div>
);
...
const PaymentsView = () => (
  <div className="space-y-6 lg:space-y-8"><div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8"><div className="bg-[#081221] border border-white/10 p-6 lg:p-8"><h3 className="ui-label text-[10px] text-muted mb-6 tracking-[0.2em]">PAYMENT SPLIT</h3><div className="h-[200px] w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={[{ name: 'GCash', value: 65 }, { name: 'On-Arrival', value: 35 }]} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value"><Cell fill="#C9943A" /><Cell fill="#ffffff10" /></Pie><Tooltip /></PieChart></ResponsiveContainer></div><div className="flex flex-col gap-2 mt-4"><div className="flex items-center gap-2"><div className="w-2 h-2 bg-gold" /><span className="ui-label text-[10px] text-white">GCASH (65%)</span></div><div className="flex items-center gap-2"><div className="w-2 h-2 bg-white/10" /><span className="ui-label text-[10px] text-white">ON-ARRIVAL (35%)</span></div></div></div><div className="lg:col-span-2 bg-[#081221] border border-white/10 p-6 lg:p-8"><h3 className="ui-label text-[10px] text-muted mb-6 tracking-[0.2em]">TRANSACTION LOG</h3><div className="space-y-4">{[{ id: 'TX-9021', name: 'Maria Santos', method: 'GCASH', amount: '₱2,400', status: 'SUCCESS', date: '10:42 AM' }, { id: 'TX-9020', name: 'James Kowalski', method: 'ON-ARRIVAL', amount: '₱4,800', status: 'PENDING', date: '10:15 AM' }, { id: 'TX-9019', name: 'Sophie Laurent', method: 'GCASH', amount: '₱1,200', status: 'SUCCESS', date: '09:55 AM' }, { id: 'TX-9018', name: 'Erik Magnusson', method: 'GCASH', amount: '₱2,400', status: 'REFUNDED', date: '08:30 AM' }].map(tx => (<div key={tx.id} className="flex items-center justify-between p-4 border border-white/5"><div className="flex items-center gap-4"><div className={`w-8 h-8 lg:w-10 lg:h-10 flex items-center justify-center border border-white/10 ${tx.status === 'REFUNDED' ? 'text-danger' : 'text-gold'}`}><CreditCard size={14} /></div><div><p className="text-white text-sm italic">{tx.name}</p><p className="ui-label text-[8px] text-muted">{tx.id} · {tx.method}</p></div></div><div className="text-right"><p className="text-white font-ui">{tx.amount}</p><p className={`ui-label text-[8px] ${tx.status === 'SUCCESS' ? 'text-success' : tx.status === 'REFUNDED' ? 'text-danger' : 'text-gold'}`}>{tx.status}</p></div></div>))}</div></div></div></div>
);

const ReportsView = () => {
  const data = [{ name: 'SHUTTLE', rev: 145000, color: '#C9943A' }, { name: 'BANGKA', rev: 89000, color: '#45B7D1' }, { name: 'PRIVATE', rev: 42000, color: '#ffffff20' }];
  return (<div className="space-y-6 lg:space-y-8"><div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8"><div className="bg-[#081221] border border-white/10 p-6 lg:p-8"><h3 className="ui-label text-[10px] text-white tracking-[0.2em] mb-8">REVENUE BY ROUTE TYPE</h3><div className="h-[250px] lg:h-[300px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} layout="vertical"><XAxis type="number" hide /><YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fill: '#8E9299', fontSize: 10 }} /><Tooltip cursor={{ fill: '#ffffff05' }} /><Bar dataKey="rev" radius={[0, 4, 4, 0]} barSize={30}>{data.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}</Bar></BarChart></ResponsiveContainer></div></div><div className="bg-[#081221] border border-white/10 p-6 lg:p-8"><h3 className="ui-label text-[10px] text-white tracking-[0.2em] mb-8">MONTHLY P&L TABLE</h3><div className="space-y-4">{[{ month: 'MARCH 2026', rev: '₱1,240,000', exp: '₱450,000', net: '₱790,000' }, { month: 'FEBRUARY 2026', rev: '₱1,120,000', exp: '₱420,000', net: '₱700,000' }, { month: 'JANUARY 2026', rev: '₱980,000', exp: '₱380,000', net: '₱600,000' }].map((row, i) => (<div key={i} className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 border border-white/5 ui-label text-[10px]"><span className="text-white col-span-2 sm:col-span-1">{row.month}</span><span className="text-muted text-right">REV: {row.rev}</span><span className="text-danger text-right">EXP: {row.exp}</span><span className="text-success text-right font-bold">NET: {row.net}</span></div>))}</div></div></div></div>);
};
...
const OperatorProfileSettings = ({ operator, onUpdate }: { operator: Operator, onUpdate: (op: Operator) => void }) => {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (errors.length > 0) {
      alert(`Upload failed:\n${errors.join('\n')}\n\nPlease resize images to under 10MB.`);
    ...
    <div className="max-w-4xl space-y-12">
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <p className="text-[10px] text-muted">Supports JPG, PNG. Max 10MB per image. Phone photos work best.</p>
        </div>
      </div>
