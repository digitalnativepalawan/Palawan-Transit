...
const DashboardView = ({ bookings, routes, onUpdateStatus, isOperatorPortal }: any) => {
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      {isOperatorPortal && <div className="bg-gold/5 border border-gold/20 p-8"><div className="max-w-xl"><h3 className="text-2xl text-white italic mb-2">Verify Reference Code</h3><p className="ui-label text-muted text-[10px] mb-6 tracking-[0.2em]">ENTER THE CUSTOMER'S REFERENCE CODE TO ACCEPT THE BOOKING</p><div className="flex gap-4"><div className="flex-1 relative"><input value={verifyCode} onChange={e => setVerifyCode(e.target.value)} placeholder="PT-2026-XXXXX" className="w-full bg-surface border border-white/10 p-4 text-white font-ui tracking-widest focus:border-gold outline-none" />{verifyError && <p className="absolute -bottom-6 left-0 text-danger ui-label text-[8px] tracking-widest">{verifyError}</p>}</div><button onClick={handleVerify} className="px-8 bg-gold text-ink ui-label text-[10px] font-bold tracking-[0.2em] hover:bg-white transition-all">ACCEPT</button></div></div></div>}
