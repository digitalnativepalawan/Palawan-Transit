      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-deep border border-border p-10 max-w-2xl w-full relative z-10">
        ...
        <div className="grid grid-cols-2 gap-6 mb-10">
          <div className="space-y-1">
            ...
            <input value={formData.from} onChange={e => setFormData({...formData, from: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">TO</label>
            <input value={formData.to} onChange={e => setFormData({...formData, to: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">TRANSPORT MODE</label>
            <select value={formData.mode} onChange={e => setFormData({...formData, mode: e.target.value as any})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none">
          ...
          <div className="space-y-1">
            ...
            <select value={formData.operator} onChange={e => setFormData({...formData, operator: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none">
              {operators.map(op => <option key={op.id} value={op.name}>{op.name}</option>)}
            </select>
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">PRICE (₱)</label>
            <input type="number" value={formData.price} onChange={e => setFormData({...formData, price: parseInt(e.target.value)})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">DEPARTURE</label>
            <input placeholder="e.g. 09:00 AM" value={formData.departureTime} onChange={e => setFormData({...formData, departureTime: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">DURATION</label>
            <input placeholder="e.g. 5 HRS" value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">SEATS</label>
            <input type="number" value={formData.seatsLeft} onChange={e => setFormData({...formData, seatsLeft: parseInt(e.target.value)})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="col-span-2 space-y-1">
            <label className="ui-label text-muted">BOOKING TYPE</label>
            <select value={formData.bookingType} onChange={e => setFormData({...formData, bookingType: e.target.value as any})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none">
              <option value="INSTANT">INSTANT CONFIRM</option>
              <option value="REQUEST">REQUEST (MANUAL APPROVAL)</option>
            </select>
          </div>
        </div>
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-deep border border-border p-10 max-w-md w-full relative z-10">
        ...
        <div className="space-y-6 mb-10">
          <div className="space-y-1">
            ...
            <input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">CONTACT PHONE</label>
            <input value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">WHATSAPP</label>
            <input value={formData.whatsapp} onChange={e => setFormData({...formData, whatsapp: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">EMAIL</label>
            <input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">PRIMARY TYPE</label>
            <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value as any})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none">
          ...
          <div className="space-y-1">
            ...
            <input value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
        </div>
        <div className="flex gap-4">
          <button onClick={onClose} className="flex-1 py-4 border border-border text-muted ui-label text-[10px] hover:text-white transition-colors">CANCEL</button>
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-deep border border-border p-10 max-w-2xl w-full relative z-10">
        ...
        <div className="grid grid-cols-2 gap-6 mb-10">
          <div className="space-y-1">
            ...
            <input value={formData.customerName} onChange={e => setFormData({...formData, customerName: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">CONTACT PHONE</label>
            <input value={formData.customerPhone} onChange={e => setFormData({...formData, customerPhone: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="col-span-2 space-y-1">
            <label className="ui-label text-muted">SELECT ROUTE</label>
            <select value={formData.routeId} onChange={e => setFormData({...formData, routeId: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none">
              {routes.map(r => <option key={r.id} value={r.id}>{r.from} → {r.to} ({r.operator})</option>)}
            </select>
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">DATE</label>
            <input type="date" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
          <div className="space-y-1">
            <label className="ui-label text-muted">SEATS</label>
            <input type="number" value={formData.seats} onChange={e => setFormData({...formData, seats: parseInt(e.target.value)})} className="w-full bg-surface border border-border p-4 text-white focus:border-gold outline-none" />
          </div>
        </div>
        <div className="flex gap-4">
          <button onClick={onClose} className="flex-1 py-4 border border-border text-muted ui-label text-[10px] hover:text-white transition-colors">CANCEL</button>
            ...
            <div className="flex gap-4">
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full bg-surface border border-white/10 p-4 text-white font-ui tracking-widest focus:border-gold outline-none"
                />
            ...
            <div key={b.id} className="bg-[#081221] border border-white/10 p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                ...
                <div className="flex justify-end items-end gap-2">
                  {b.status === 'PENDING' && (
                    <>
                      <button onClick={() => onUpdateStatus(b.id, 'ACCEPTED')} className="p-2 text-success hover:bg-success/10 transition-colors"><Check size={16} /></button>
                      <button onClick={() => onUpdateStatus(b.id, 'CANCELLED')} className="p-2 text-danger hover:bg-danger/10 transition-colors"><X size={16} /></button>
                    </>
                  )}
...
const RoutesView = ({ routes, onAdd, onEdit, onDelete }: any) => (
  <div className="space-y-6 lg:space-y-8">
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      ...
      <button onClick={onAdd} className="w-full sm:w-auto bg-gold text-ink px-8 py-3 ui-label text-[11px] font-bold tracking-[0.2em] flex items-center justify-center gap-2">
      ...
      <div className="lg:hidden space-y-4">
        {passengers.map((p, i) => (
          <div key={i} className="bg-[#081221] border border-white/10 p-6 space-y-4">
            <div className="flex justify-between items-start">
              <div><p className="text-white text-lg italic">{p.name}</p><p className="ui-label text-[8px] text-muted">{p.phone}</p></div>
              <button className="p-2 text-muted hover:text-white"><MoreVertical size={16} /></button>
            </div>
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
          ...
          {[
            ...
            <div key={i} className="space-y-2">
              ...
              <div className="flex gap-2">
                <input type="password" value={key.value} readOnly className="flex-1 bg-[#081221] border border-white/10 p-3 ui-label text-[10px] text-white outline-none" />
...
const OperatorProfileSettings = ({ operator, onUpdate }: { operator: Operator, onUpdate: (op: any) => void }) => {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      const f = files[i];
      if (f.size > MAX_SIZE) errors.push(`${f.name} over 10MB`);
      else if (!f.type.startsWith('image/')) errors.push(`${f.name} not an image`);
      else valid.push(f);
    }
    if (errors.length > 0) alert(`Skipped:\n${errors.join('\n')}`);
    if (valid.length === 0) return;
    setUploadingPhotos(true);
    const newPhotos = [...vehiclePhotos];
    for (const file of valid) {
      const ext = file.name.split('.').pop();
      const path = `${operator.id}-${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
            ...
            <div key={key} className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full bg-[#081221] border border-white/10 ${icon ? 'pl-12' : 'pl-4'} pr-4 py-4 ui-label text-[11px] text-white outline-none focus:border-gold transition-colors`}
                />
              </div>
            </div>
          ))}
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-end">
          <div>
            ...
            <p className="text-sm text-muted">Photos of your vans, 4x4s, bangkas, or island hopping boats</p>
          </div>
            ...
            {vehiclePhotos.map((photo, idx) => (
              <div key={idx} className="relative group aspect-video bg-[#081221] border border-white/10 overflow-hidden">
                ...
                <button
                  ...
                  className="absolute top-2 right-2 p-1 bg-black/70 border border-danger/50 opacity-0 group-hover:opacity-100 transition-opacity"
                >
        ...
        <div className="flex items-center gap-4">
          ...
          <label
            ...
            className={`cursor-pointer border border-white/20 px-6 py-3 ui-label text-[11px] text-white hover:border-gold transition-all flex items-center gap-2 ${uploadingPhotos ? 'opacity-50 pointer-events-none' : ''}`}
          >
            {uploadingPhotos ? <Clock size={16} className="animate-spin" /> : <Camera size={16} />}
            {uploadingPhotos ? 'UPLOADING...' : 'ADD PHOTOS'}
          </label>
          <p className="text-[10px] text-muted">JPG or PNG · Max 10MB each · Multiple allowed</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-end">
          <div>
            ...
            <p className="text-sm text-muted">Upload accreditation documents for your operation</p>
          </div>
            ...
            {permits.map((permit, idx) => (
              <div key={idx} className="flex items-center justify-between p-4 bg-[#081221] border border-white/10 group hover:border-gold/30 transition-all">
                ...
                <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                  <a href={permit.url} target="_blank" rel="noreferrer" className="p-2 text-muted hover:text-gold transition-colors">
        ...
        <div className="p-6 bg-[#081221] border border-white/10 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <select
              ...
              className="flex-1 bg-[#050B14] border border-white/10 p-3 ui-label text-[11px] text-white outline-none focus:border-gold"
            >
              {PERMIT_TYPES.map(p => (
            ...
            <label
              ...
              className={`cursor-pointer border border-white/20 px-6 py-3 ui-label text-[11px] text-white hover:border-gold transition-all flex items-center justify-center gap-2 whitespace-nowrap ${uploadingPermit ? 'opacity-50 pointer-events-none' : ''}`}
            >
      ...
      <div className="p-8 bg-gold/5 border border-gold/20 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-[11px] text-muted leading-relaxed">
            Vehicle photos and verified permits are displayed on your public profile and increase traveler trust. All documents are stored securely in Supabase Storage.
          </p>
        </div>
      </div>
    </div>
